...
从这句`17/12/25 18:28:00 INFO yarn.Client: Requesting a new application from cluster with 0 NodeManagers` 得到从集群中没有获得NodeManagers，检查Hadoop集群是否启动正常。发现进程中没有 `NodeManager` ，原因是，在启动的时候，提示没有JAVA_HOME（`JAVA_HOME is not set and could not be found`），解决方案
